...
class LeRobotRoboMemoryDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[robomemory_policy.RoboMemoryInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
        ...
        return dataclasses.replace(
            ...
            data_transforms=data_transforms,
            model_transforms=model_transforms,
            use_quantile_norm=model_config.model_type == ModelType.PI0_FAST, # TODO: check this
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi0_fast_robomemory_low_mem_finetune",
        model=pi0_fast.Pi0FASTConfig(
            action_dim=8, action_horizon=10, max_token_len=180, paligemma_variant="gemma_2b_lora"
        ),
        data=LeRobotRoboMemoryDataConfig(
            repo_id="jennypan00/pi0_fast_ft_droid_lerobot",
            base_config=DataConfig(prompt_from_task=True),
            assets=AssetsConfig( # TODO: check this
                assets_dir="/iris/u/jrpan/openpi/assets",
                asset_id="pi0_fast_robomemory_low_mem_finetune/jennypan00/pi0_fast_ft_droid_lerobot",
            ),
        ),
    ...
    TrainConfig(
        name="pi0_fast_robomemory_finetune",
        model=pi0_fast.Pi0FASTConfig(
        ...
        data=LeRobotRoboMemoryDataConfig(
            ...
            base_config=DataConfig(prompt_from_task=True),
            assets=AssetsConfig( # TODO: check this
                assets_dir= "/iris/u/jrpan/openpi/assets", # TODO: check "gs://openpi-assets/checkpoints/pi0_fast_base/assets",
                asset_id="pi0_fast_robomemory_finetune/jennypan00/pi0_fast_ft_droid_lerobot",
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_fast_droid/params"),
        num_train_steps=30_000,
